...
`datagen` plays the engine against itself and writes the bulletformat records
that `tools/trainer` learns from. One run is one shard:

    cargo run --release --bin datagen -- <output> <positions> [threads] [nodes] [seed]
